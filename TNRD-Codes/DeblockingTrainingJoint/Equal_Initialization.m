...
load training_deblocking_7x7_400_176x176_greedy_stage=5.mat;
x0 = cof(:,1:run_stage);
...
w = repmat(w,[1,filtN]);
cof_beta = eye(m,m);
cof_beta = cof_beta(:,end-filtN+1:end);
x0 = zeros(length(cof_beta(:)) + filtN*mfs.NumW, run_stage);
theta = [10 5 ones(1,run_stage-2)];

for i=1:run_stage
    x0(:,i) = [cof_beta(:); w(:)*theta(i)];
end
function grad = grad_l_thetas(AllNetWorks, trained_model, input, basis, R, psz, KernelPara, mfs, s, reaction_term)
NumW = mfs.NumW;
...
grad_loss_p = -p*sum(sum(reaction_term.*grad_l_u));
% grad_loss_p = -p*sum(sum(A_t*(A*input - noisy).*grad_l_x));
